...
def ensure_board() -> Tuple[bool, str]:
    """确保备忘看板 Bitable 已创建。返回 (ok, url_or_error)。"""
...
def refresh_board(
    ...
    if ok_list and existing:
        old_ids = [r["record_id"] for r in existing if r.get("record_id")]
        if old_ids:
            batch_delete_bitable_records(app, tid, old_ids)

    for row in rows:
        raw_status = row[2] if len(row) > 2 else "进行中"
        status = _STATUS_MAP.get(raw_status, "进行中")
        partition = row[4] if len(row) > 4 else "今日新增"
        fields = {
            "线程": row[0] if row else "",
            "内容": row[1] if len(row) > 1 else "",
            "状态": status,
            "创建时间": row[3] if len(row) > 3 else "",
            "分区": partition,
        }
        add_bitable_record(app, tid, fields)

    _log(f"看板刷新完成: {len(rows)} 条记录")
    return True, url, stats
