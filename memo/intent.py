...
# 自然语言前缀："帮我把这段翻成英文" / "帮把下面内容翻成中文"
_TRANS_NAT_PAT = re.compile(
    r"^(?:帮我?)?(?:把)?(?:这段|这个|下面|以下)?(?:话|内容|文字|文案|段落)?(?:翻译?成?|转成?)(英文|英语|中文|中文版)?\s*[：:]?\s*(.+)$",
    re.DOTALL,
)
# "用英文怎么说 xxx"
...
# 末尾触发："xxx 翻译一下" / "xxx 翻成英文" / "xxx 的英文"
...
def _detect_translate(text: str) -> Optional[tuple[str, dict]]:
    """检测翻译意图，覆盖多种自然表达。匹配到返回 ("translate", params)，否则 None。"""
    t = text.strip()
    content = ""
    target_lang = ""
    hint = ""
...
29. 翻译 - 用户说「翻译 xxx」「translate xxx」「帮我翻 xxx」「翻成英文 xxx」「翻成中文 xxx」「英文版 xxx」「中文版 xxx」「英译中 xxx」「中译英 xxx」「用英文怎么说 xxx」。action: translate，params: content（要翻译的内容）, target_lang（"en"/"zh"/空=自动判断）, scene（使用场景：email/ppt/slack/public，可为空）。
30. 普通聊天 - 以上都不是。action: chat，reply: 你的简短回复。
...
- 翻译：{"action":"translate","params":{"content":"我们希望在Q2完成创作者合作流程的优化","target_lang":"en","scene":"ppt"},"reply":""}
